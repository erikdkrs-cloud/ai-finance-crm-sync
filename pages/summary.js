    ...
    <div className={"top-card " + variant} style={{ animationDelay: index * 120 + "ms" }}>
      ...
      <div className="top-card-info">
        ...
        <div className="top-card-meta">
          ...
          <span>Маржа: {fmtPct(Number(project.margin) || 0)}</span>
        </div>
      </div>
    ...
    <div className="expense-bar-widget">
      <div className="expense-stacked-bar">
        {items.map(function (item, i) {
          ...
          return (
            <div key={i} className="expense-bar-segment"
              style={{ width: pct + "%", background: item.color }}
              title={item.label + ": " + fmtMoney(item.value, 0) + " ₽"} />
          );
        })}
      </div>
...
export default function Summary() {
  ...
  var riskDist = data && data.riskDistribution
        ...
        <div className="summary-page-header">
          ...
          {months.length > 0 && (
            <div className="dashboard-period-bar" style={{ margin: 0 }}>
              ...
              <select className="dkrs-select" value={selectedMonth}
            ...
            <div className="summary-two-cols">
              <div className="summary-widget glass-card">
                ...
                <div className="top-cards-list">
                  {(data.top_profitable || []).map(function (p, i) {
                    return <TopCard key={p.project} project={p} index={i} variant="profit" />;
                  })}
                  {(!data.top_profitable || data.top_profitable.length === 0) && (
                    <div className="empty-widget">Нет данных</div>
                  )}
                </div>
              </div>
              <div className="summary-widget glass-card">
                ...
                <div className="top-cards-list">
                  {(data.top_unprofitable || []).map(function (p, i) {
                    return <TopCard key={p.project} project={p} index={i} variant="loss" />;
                  })}
                  {(!data.top_unprofitable || data.top_unprofitable.length === 0) && (
                    <div className="empty-widget">Нет убыточных</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="summary-two-cols">
              ...
              <div className="summary-widget glass-card">
                <div className="widget-header">
                  ...
                  <span className="widget-badge neutral">
                    {fmtMoney((data.totals && data.totals.costs) || 0, 0)} ₽
                  </span>
                </div>
            ...
            <div className="summary-widget glass-card full-width">
              <div className="widget-header">
                ...
                <span className="widget-badge orange">
                  {data.anomalies ? data.anomalies.length : 0} найдено
                </span>
              </div>
                ...
                <table className="dkrs-table">
                  ...
                  <tbody>
                    {data.anomalies && data.anomalies.length > 0 ? (
                      data.anomalies.map(function (item, i) {
                        var icon = "📉";
                        if (item.reason === "Штрафы") icon = "⚖️";
                        if (item.reason === "Высокие расходы на рекламу") icon = "📢";
                        var isMargin = item.reason && item.reason.indexOf("маржинальность") !== -1;
                        return (
                          <tr key={i} className="table-row-animated" style={{ animationDelay: i * 50 + "ms" }}>
                            <td className="project-name">{item.project}</td>
                            <td><span className="anomaly-chip">{icon} {item.reason}</span></td>
                            <td className={isMargin ? "negative-value" : ""}>{formatAnomalyValue(item)}</td>
                          </tr>
                        );
                      })
                    ) : (
                      <tr>
                        <td colSpan="3" className="empty-state">
                          <div className="empty-icon">🎉</div>Аномалий не найдено!
                        </td>
                      </tr>
                    )}
                  </tbody>
              ...
              <div className="summary-widget glass-card">
                ...
                <div className="prompts-grid">
                  {[
                    ...
                    return (
                      <button key={i} className="prompt-tag"
                        onClick={function () { router.push("/assistant"); }}
