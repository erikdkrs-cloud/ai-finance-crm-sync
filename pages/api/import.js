...
export default async function handler(req, res) {
  if (req.method !== "POST") {
    return res.status(405).json({ ok: false, where: "import", method: req.method });
  }
  ...
  // Быстрый тест — чтобы сразу увидеть, что POST точно сюда попал
  if (rows.length === 0) return res.status(200).json({ ok: true, imported: 0, note: "empty rows received" });
