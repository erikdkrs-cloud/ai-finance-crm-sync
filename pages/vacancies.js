      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        {tab === "vacancies" && !selectedVacancy && (
          <div>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", background: "#fff", padding: "14px 16px", borderRadius: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                <thead>
                  <tr style={{ background: "#f8fafc", borderBottom: "2px solid #e5e7eb" }}>
                    ...
                    <th style={{ padding: "14px 12px", textAlign: "center", width: 110 }}>Зарплата</th>
                    <th style={{ padding: "14px 12px", textAlign: "center", width: 80 }}>👁</th>
                    <th style={{ padding: "14px 12px", textAlign: "center", width: 80 }}>💬</th>
                    <th style={{ padding: "14px 12px", textAlign: "center", width: 100 }}>Статус</th>
                    <th style={{ padding: "14px 12px", textAlign: "center", width: 100 }}>Дата</th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    ...
                    return (
                      <tr key={v.id} onClick={function () { setSelectedVacancy(v); }}
                        ...
                        <td style={{ padding: "14px 12px", textAlign: "center", fontSize: 12, color: "#888" }}>{formatDateShort(v.created_at)}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        {tab === "vacancies" && selectedVacancy && (
          <div>
            ...
            <h3>💬 Отклики ({vacResponses.length})</h3>
            {vacResponses.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {vacResponses.map(function (r) {
                  ...
                  return (
                    <div key={r.id} style={{ background: "#fff", borderRadius: 14, padding: "16px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: "1px solid #f0f0f0" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{ width: 40, height: 40, borderRadius: "50%", background: "#ede9fe", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#7c3aed", fontSize: 16 }}>{(r.author_name || "?")[0].toUpperCase()}</div>
                          <div>
                      ...
                      {r.message && <div style={{ fontSize: 13, color: "#555", whiteSpace: "pre-line", maxHeight: 120, overflow: "auto", background: "#f9fafb", borderRadius: 10, padding: 12 }}>{r.message}</div>}
                    </div>
                  );
                })}
              </div>
                ...
                {filteredResponses.slice(0, 50).map(function (r) {
                  ...
                  return (
                    <div key={r.id} style={{ background: "#fff", borderRadius: 14, padding: "16px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: "1px solid #f0f0f0" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{ width: 42, height: 42, borderRadius: "50%", background: "#ede9fe", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#7c3aed", fontSize: 17 }}>{(r.author_name || "?")[0].toUpperCase()}</div>
                          <div>
                            ...
                            {raw.location && <div style={{ fontSize: 12, color: "#aaa" }}>📍 {raw.location}</div>}
                          </div>
                        </div>
                        <div style={{ fontSize: 12, color: "#888" }}>{formatDate(r.created_at)}</div>
                      </div>
                      {r.message && <div style={{ fontSize: 13, color: "#555", whiteSpace: "pre-line", maxHeight: 80, overflow: "hidden", background: "#f9fafb", borderRadius: 10, padding: 12 }}>{r.message.slice(0, 300)}{r.message.length > 300 ? "..." : ""}</div>}
                    </div>
                  );
                })}
              </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
                  <div>
                    ...
                    <input value={accForm.name} onChange={function (e) { setAccForm(Object.assign({}, accForm, { name: e.target.value })); }}
                      placeholder="Мой аккаунт" required style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "1px solid #e5e7eb", fontSize: 14, boxSizing: "border-box" }} />
                  </div>
                  <div>
